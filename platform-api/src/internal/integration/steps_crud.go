...
func (w *world) revokedTokenState(status, by string) error {
	...
	wantStatus := constants.GatewayTokenStatusRevoked
	if gone == nil || string(gone.Status) != string(wantStatus) || gone.RevokedBy == nil || *gone.RevokedBy != by {
		return fmt.Errorf("[%s] revoked token state mismatch: %+v", w.it.driver, gone)
	}
	return nil
}
