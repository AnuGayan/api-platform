...
func (w *world) pageWebSubAPIs(pageSize, total int) error {
	websubRepo := repository.NewWebSubAPIRepo(w.it.db)
	page1, err := websubRepo.List(w.orgID, "", pageSize, 0)
	if err != nil {
		return fmt.Errorf("[%s] List(%d,0) failed: %w", w.it.driver, pageSize, err)
	}
	if len(page1) != pageSize {
		return fmt.Errorf("[%s] List(%d,0): want %d, got %d", w.it.driver, pageSize, pageSize, len(page1))
	}
	page2, err := websubRepo.List(w.orgID, "", pageSize, pageSize)
	if err != nil {
		return fmt.Errorf("[%s] List(%d,%d) failed: %w", w.it.driver, pageSize, pageSize, err)
	}
	if len(page2) != pageSize {
		return fmt.Errorf("[%s] List(%d,%d): want %d, got %d", w.it.driver, pageSize, pageSize, pageSize, len(page2))
	}
	seen := map[string]bool{}
	for _, a := range append(page1, page2...) {
		if seen[a.UUID] {
			return fmt.Errorf("[%s] pagination overlap: UUID %s seen twice", w.it.driver, a.UUID)
		}
		seen[a.UUID] = true
	}
	if len(seen) != total {
