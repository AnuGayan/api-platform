...
func RegisterLLMSteps(ctx *godog.ScenarioContext, state *TestState, httpSteps *steps.HTTPSteps) {
	...
	// Policy engine route metadata assertion for provider_name
	ctx.Step(`^the policy engine route metadata should contain provider_name "([^"]*)"$`, func(expectedProviderName string) error {
		body := httpSteps.LastBody()
		if len(body) == 0 {
			return fmt.Errorf("expected non-empty response body")
		}

		return assertRouteMetadataContainsProviderName(body, expectedProviderName)
	})
...
// assertRouteMetadataContainsProviderName checks that the policy engine config dump
// contains the expected provider_name in the route_metadata section.
func assertRouteMetadataContainsProviderName(body []byte, expectedProviderName string) error {
	var configDump struct {
		RouteMetadata struct {
			Routes []struct {
	...
	if err := json.Unmarshal(body, &configDump); err != nil {
		return fmt.Errorf("failed to parse policy engine config dump: %w", err)
	}

	for _, route := range configDump.RouteMetadata.Routes {
		if route.ProviderName == expectedProviderName {
			return nil
		}
	}

	return fmt.Errorf("provider_name %q not found in policy engine route_metadata", expectedProviderName)
}
